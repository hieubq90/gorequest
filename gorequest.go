...
func (s *SuperAgent) queryStruct(content interface{}) *SuperAgent {
	if marshalContent, err := jsoniter.Marshal(content); err != nil {
		s.Errors = append(s.Errors, err)
	} else {
		var val map[string]interface{}
		if err := jsoniter.Unmarshal(marshalContent, &val); err != nil {
			s.Errors = append(s.Errors, err)
		} else {
			for k, v := range val {
				k = strings.ToLower(k)
...
func (s *SuperAgent) queryString(content string) *SuperAgent {
	...
	if err := jsoniter.Unmarshal([]byte(content), &val); err == nil {
		for k, v := range val {
			s.QueryData.Add(k, v)
		}
	} else {
...
func (s *SuperAgent) SendStruct(content interface{}) *SuperAgent {
	if marshalContent, err := jsoniter.Marshal(content); err != nil {
		...
		var val map[string]interface{}
		d := jsoniter.NewDecoder(bytes.NewBuffer(marshalContent))
		d.UseNumber()
...
func (s *SuperAgent) SendString(content string) *SuperAgent {
	if !s.BounceToRawString {
		var val interface{}
		d := jsoniter.NewDecoder(strings.NewReader(content))
		d.UseNumber()
		if err := d.Decode(&val); err == nil {
...
func changeMapToURLValues(data map[string]interface{}) url.Values {
	...
	for k, v := range data {
		...
		case bool:
			...
			// jsoniter.Number used to protect against a wrong (for GoRequest) default conversion
		...
		case []interface{}:
			...
			case jsoniter.Number:
				for _, element := range val {
					newUrlValues.Add(k, string(element.(jsoniter.Number)))
				}
			}
		default:
			// TODO add ptr, arrays, ...
...
func (s *SuperAgent) MakeRequest() (*http.Request, error) {
	...
	case TypeJSON:
		...
		if s.BounceToRawString {
			...
			contentJson, _ = jsoniter.Marshal(s.Data)
		} else if len(s.SliceData) != 0 {
			contentJson, _ = jsoniter.Marshal(s.SliceData)
		}
		if contentJson != nil {
	...
	case TypeMultipart:
		...
		if len(s.SliceData) != 0 {
			...
			contentJson, err := jsoniter.Marshal(s.SliceData)
			if err != nil {
				return nil, err
			}
